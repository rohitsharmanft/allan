...
const ProjectListing = () => {
    ...
    if (searchTerm.trim()) {
      ...
      filtered = filtered.filter((job) => {
        return (
          ...
          (job.projectDescription || "").toLowerCase().includes(lowerSearch) ||
          (job.category?.title || "").toLowerCase().includes(lowerSearch) ||
          ...
          if (response.data.code === 200) {
            ...
            if (newJobs.length > 0) {
              setJobs((prev) => {
                ...
                const uniqueNew = newJobs.filter(
                  (j) => !existingIds.has(j._id),
                );
  ...
  const getSkillTitle = (job) => {
    if (job.skillData && job.skillData.length > 0) {
      return job.skillData[0].title;
    }
        ...
        {!token && (
          <div className="job-info-lines">
            <p>
              ...
              <span className="info-role">Clients</span> – You can post a{" "}
              {type === "job" ? "job" : "project"} when you{" "}
            ...
            <p>
              ...
              <span className="info-role">Members</span> – You can provide a
              quotation for a project when you{" "}
              <span className="info-link" onClick={() => navigate("/login")}>
                LOGIN
              </span>{" "}
              or{" "}
        ...
        <div className="job-search-container">
          ...
          {!isMember && (
            <GradientButton
              ...
              onClick={() => navigate(`/post-project`, { state: { type } })}
            >
            ...
            <>
              {filteredJobs.map((job) => (
                <JobCard
                  ...
                  onClickReadMore={() => {
                    if (!token && type === "job") {
                      // Jobs: let guests see the detail page — redirect happens on "Apply" click
                      navigate(`/${type}-detail`, { state: { job, type } });
                    } else if (!token && type === "project") {
                      // Projects: go to detail page — blur overlay will handle the lock
