...
const MemberJobsPage = () => {
    ...
    const fetchMemberJobs = async (page = 1, pageSize = 10) => {
        ...
        try {
            const response = await axios.post(get_member_jobs, {type:'job'}, {
    ...
    const handleTableChange = (page, pageSize) => {
        fetchMemberJobs(page, pageSize);
    };

    const handleView = (record) => {
        navigate(`/member-job-details`, { state: { job: record.raw } });
    };

    const renderClientActionStatus = (status) => {
        let color = "#faad14";
