...
const member_seckret_key = seckret_keys.member_seckret_key;
...
const generate_token = (token_data: any) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      if (token_data.scope == "member") {
        seckret_key = member_seckret_key;
      }
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (type == "admin") {
        ...
        set_seckret_key = member_seckret_key;
      }
...
const verify_token = async (token: string, type: string, language: string) => {
  try {
    ...
    if (decoded.scope == "member") {
      let query: any = {
        member_id: decoded._id,
        access_token: { $ne: null },
        token_gen_at: decoded.token_gen_at,
      };
      let projection = { __v: 0 };
